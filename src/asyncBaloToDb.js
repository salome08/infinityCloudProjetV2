...
const definingRcs = async (issuerId, text) => {
  try {
    const RCSrgx = RegExp(
    ...
    if (RCSrgx.test(text)) {
      ...
      // society.rcs = rcs[0];
      Society.insertRcs(issuerId, rcs[0])
      // return true;
    }
    // else return false;
  } catch (error) {
...
const definingBoard = async (issuerId, text) => {
  try {
    if (text.search(/conseil d'administration/i) > 1) {
      Society.insertBoard(issuerId, "CA")
    }
    else if (text.search(/directoire et conseil/i) > 1) {
      Society.insertBoard(issuerId, "DCS")
    }
  } catch (error) {
...
const definingCorporateForm = async (issuerId, text) => {
  try {
    if (text.search(/Société anonyme/i) > 0) {
      Society.insertCorporateform(issuerId, "SA")
    } else if (text.search(/Société europ/i) > 0) {
      Society.insertCorporateform(issuerId, "SE")
    } else if (text.search(/commandite/i) > 0) {
      Society.insertCorporateform(issuerId, "SCA")
    }
  } catch (error) {
  ...
  try {
    ...
    textract.fromFileWithPath(filePath, async (error, text) => {
      if (error) console.error("textrac error : ", filePath);
        ...
        const issuerId = await definingName(twiceSlicedText);
        if (issuerId) {
          await definingRcs(issuerId, slicedText);
          // await definingDate(issuerId, text);
          await definingCorporateForm(issuerId, text);
          await definingBoard(issuerId, text);
          // await insertIntoDb(issuerId);
          outList.push({
            File: file,
            Name: society.name,
